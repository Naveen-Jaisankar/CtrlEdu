...
const ModuleTab: React.FC = () => {
  ...
  useEffect(() => {
    if (
      currentTeacherId &&
      !allTeachers.some((teacher) => teacher.teacherId === currentTeacherId)
    ) {
  ...
  const fetchAvailableTeachersForAdd = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8084/api/admin/available-teachers`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
          },
  ...
  const fetchAllTeachersForEdit = async () => {
    ...
    try {
      ...
      const updatedTeachers = teachers.some(
        (teacher) => teacher.teacherId === currentTeacher.teacherId
      )
        ? teachers
  ...
  const handleDeleteModule = async (moduleId: number) => {
    try {
      await axios.delete(
        `http://localhost:8084/api/admin/delete-module/${moduleId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
          },
    ...
    try {
      ...
      if (editingModule) {
        ...
        const isDuplicate = modules.some(
          (mod) =>
            mod.teacherId === teacher && mod.moduleId !== editingModule.moduleId
        );
        if (isDuplicate) {
        ...
        // Add a new module
        await axios.post(
          "http://localhost:8084/api/admin/add-module",
          payload,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
