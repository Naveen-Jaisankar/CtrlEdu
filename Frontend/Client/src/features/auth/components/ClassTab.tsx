...
const ClassTab: React.FC = () => {
  ...
  const fetchClasses = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8084/api/admin/classes",
        {
          headers: {
  ...
  const fetchStudents = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8084/api/admin/students",
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
          },
  ...
  const fetchModules = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8084/api/admin/modules",
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
          },
  ...
  const handleAddOrEditClass = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingClass) {
        // Edit Class API call
        await axios.put(
          `http://localhost:8084/api/admin/edit-class/${editingClass.classId}`,
          payload,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
  ...
  const handleEditClick = (cls) => {
    ...
    const available = students.filter(
      (student) =>
        !selectedStudentIds.includes(student.id) && !student.isAssigned
    );
  ...
  const handleDeleteClass = async (classId: number) => {
    ...
    try {
      await axios.delete(
        `http://localhost:8084/api/admin/delete-class/${classId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
          },
    ...
    <div className="p-6">
      ...
      >
        ...
        <form onSubmit={handleAddOrEditClass}>
          ...
          <div
            style={{
              color: selectedStudents.length >= numStudents ? "red" : "black",
            }}
          >
            {selectedStudents.length >= numStudents
              ? "You have reached the maximum number of students."
              : `You can select ${
                  numStudents - selectedStudents.length
                } more students.`}
          </div>

          <div
            className="dual-list-box-container"
